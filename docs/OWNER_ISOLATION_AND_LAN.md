...
6. 携带新配对 token 的链接优先于页面内仍存的旧会话：页面复用（仅 hash 变化的导航）或重新打开时都会先清理旧会话再兑换新 token，被撤销的旧 session 不会继续展示。
7. 手机端 LAN session credential 只保存在页面内存中，不写入 sessionStorage/localStorage 等浏览器存储；刷新页面后须使用新配对链接重新兑换。
